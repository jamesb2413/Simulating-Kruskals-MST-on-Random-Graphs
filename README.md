...
To find MSTs, I used Kruskal’s algorithm because Kruskal’s runs faster than Prim’s on sparse graphs. Initially, I used Prim’s MST Algorithm because I reasoned that complete graphs are very dense. However, I finally realized that pruning edges makes the graphs much more sparse. Using Kruskal’s, I solved the problems with memory and had much faster run time, consistent with theory. This project taught me to seriously consider memory and time limitations before implementing solutions. I spent hours reasoning about the run times of Prim’s and Kruskal’s for this specific application, but I completely overlooked considerations of memory. 

I also learned a lot about modifying data structures to use less memory. My graph representations evolved from an adjacency matrix for faster run time to an adjacency list, to a Java ArrayList to save memory. I learned that for a very specific application like this, we can leave a lot of information behind and only store key values for later computations.
